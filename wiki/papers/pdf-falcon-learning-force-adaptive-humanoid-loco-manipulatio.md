# [PDF] FALCON: Learning Force-Adaptive Humanoid Loco-Manipulation

**Venue**: arXiv preprint arXiv …, 2025  |  **Category**: [[topics/robotics|Robotics]]

**Authors**: Y Zhang, Y Yuan, P Gurunath, T He, S Omidshafiei, …

**Followed researchers**: [[researchers/ali-agha|Ali Agha]]

**Topics**: [[topics/manipulation|Manipulation]], [[topics/locomotion|Locomotion]]

---

## Summary

_No snippet available in the source alert._

## Related

- [[papers/pdf-world-model-failure-classification-and-anomaly-detection|(PDF) World Model Failure Classification and Anomaly Detection for Autonomous Inspection]]
- [[papers/2604.23073|RL Token: Bootstrapping Online RL with Vision-Language-Action Models]]
- [[papers/2606.05687|Accelerating and Scaling MPC-Guided Reinforcement Learning for Humanoid Locomotion and Manipulation]]
- [[papers/2606.06218|TAM: Torque Adaptation Module for Robust Motion Transfer in Manipulation]]
- [[papers/2606.18610|SC3-Eval: Evaluating Robot Foundation Models via Self-Consistent Video Generation]]
- [[papers/2606.21148|Pose-Agnostic Robotic Functional Grasping via Observation-Action Canonicalization]]
- [[papers/2606.30988|Multisensory Continual Learning: Adapting Pretrained Visuomotor Policies to Force]]
- [[papers/2606.32027|Freeform Preference Learning for Robotic Manipulation]]

## Source (raw)

- [[sources/agha-0517-eml|agha_0517.eml]]

> The raw alert email is the source of truth and is never edited. This page is compiled from it.

---
*Compiled by `src/wiki.py` from the raw `email_src/` alerts — 2026-07-08 11:34 EDT. Do not hand-edit; edit the raw emails instead.*