# [PDF] Accelerating Signal-Temporal-Logic-Based Task and Motion Planning of Bipedal Navigation using Benders Decomposition

**Venue**: arXiv preprint arXiv …, 2025  |  **Category**: [[topics/robotics|Robotics]]

**Authors**: J Ren, X Lin, R Mineyev, KM Feigh, [[researchers/samuel-coogan|Samuel Coogan]], Y Zhao

**Followed researchers**: [[researchers/samuel-coogan|Samuel Coogan]]

**Topics**: [[topics/locomotion|Locomotion]], [[topics/navigation-and-planning|Navigation And Planning]]

---

## Summary

_No snippet available in the source alert._

## Related

- [[papers/2509.19484|linrax: A JAX Compatible, Simplex Method Linear Program Solver]]
- [[papers/2606.00985|Make Your VLA More Robust Without More Data By Interleaving Motion Planning]]
- [[papers/2606.19590|Safe, Real-Time Active Model Discrimination and Fault Diagnosis for Nonlinear Systems via Differentiable Reachability]]
- [[papers/advances-in-contraction-theory-for-robust-optimization-contr|Advances in Contraction Theory for Robust Optimization, Control, and Neural Computation]]
- [[papers/pdf-accelerating-large-scale-temporal-logic-based-task-and-m|(PDF) Accelerating Large-scale Temporal-Logic-based Task and Motion Planning with Logic Network Flow]]
- [[papers/pdf-seeing-saying-solving-an-llm-to-tl-framework-for-coopera|(PDF) Seeing, Saying, Solving: An LLM-to-TL Framework for Cooperative Robots]]
- [[papers/arch-comp26-category-report-artificial-intelligence-and-neur|ARCH-COMP26 Category Report: Artificial Intelligence and Neural Network Control Systems (AINNCS) for Continuous and Hybrid Systems Plants]]
- [[papers/low-discrepancy-perturbation-design-for-numerically-stable-g|Low-Discrepancy Perturbation Design for Numerically Stable Gradient Estimation in Feedback Optimization]]

## Source (raw)

- [[sources/coogan-0824-eml|coogan_0824.eml]]

> The raw alert email is the source of truth and is never edited. This page is compiled from it.

---
*Compiled by `src/wiki.py` from the raw `email_src/` alerts — 2026-07-08 10:59 EDT. Do not hand-edit; edit the raw emails instead.*